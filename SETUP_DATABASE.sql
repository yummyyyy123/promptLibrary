-- SMS OTP Database Schema - RUN THIS IN SUPABASE SQL EDITOR
...
-- RLS Policies (allow all for simplicity in this case)
CREATE POLICY "Allow all operations on admin_phone_numbers" ON admin_phone_numbers
    FOR ALL USING (true);

CREATE POLICY "Allow all operations on sms_otps" ON sms_otps
    FOR ALL USING (true);
...
SELECT 'admin_phone_numbers table created' as status;
SELECT 'sms_otps table created' as status;
